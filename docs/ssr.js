...
export function App () {
  ...
  return div(
    ...
    p('On the server, use ', code('renderToString'), ' to render it to HTML:'),
    prismCode(
`// server.mjs
...
<body>
  ...
  <script type="module" src="/client.mjs"></script>
    ...
    p('On the client, call ', code('hydrate'), ' to make the page interactive:'),
    prismCode(
...
const items = registeredState('items', ['one', 'two', 'three'])`
    ...
    prismCode(
`const { state } = await renderToString({ url: '/', appFn: () => App() })
...
`await renderToString({
  ...
  contentSection(
    'Using with Spliffy',
    p(a({ href: 'https://github.com/SRFNStack/spliffy' }, '@srfnstack/spliffy'), ' is a directory-based Node.js web framework that pairs naturally with fntags SSR. Here\'s a minimal setup:'),
    prismCode(
`// server.mjs
...
spliffy({
  ...
  port: 3000
    ...
    p('Create a catch-all route handler that server-renders your app. The ', code('+'), ' suffix in spliffy means this handler matches all paths under ', code('/'), ':'),
    prismCode(
`// www/index+.rt.mjs — handles all page requests
import { renderToString, escapeScriptContent } from '@srfnstack/fntags-ssr'
import { App } from '../app.mjs'
...
<body>
  ...
  <script type="module" src="/client.mjs"></script>
    ...
    p('Place your client entry file in the same ', code('www/'), ' directory. Spliffy will serve it as a static file:'),
    prismCode(
`// www/client.mjs — served as static ES module
import { hydrate } from '@srfnstack/fntags-ssr/hydrate'
import { App } from '../app.mjs'
...
const page1 = await renderToString({
  url: '/',
  appFn: () => App()  // registeredState('x', 'first')
})

const page2 = await renderToString({
  url: '/',
  appFn: () => App()  // registeredState('x', 'second')
})
