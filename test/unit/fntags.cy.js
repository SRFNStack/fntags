...
describe('fntags', () => {
  ...
  describe('renderNode', () => {
    it('assumes a node if it\'s a truthy object without a then function', () => {
      const o = { id: 1 }
      const n = fntags.renderNode(o)
      expect(n).eq(o)
    })
    it('converts strings and numbers into text nodes', () => {
      const a = fntags.renderNode('a')
      const i = fntags.renderNode(1)
  ...
  describe('getAttrs', () => {
    it('returns the first element of the passed array if it\'s an attributes object', () => {
      expect(getAttrs([{ foo: 'bar' }])).to.deep.equal({ foo: 'bar' })
    })
    it('doesn\'t have time for your bs', () => {
      for (const bad in [null, undefined, 'taco', {}, () => {}, 0, NaN, Infinity, []]) {
        expect(getAttrs(bad)).to.deep.equal({})
      }
    })
  })
  ...
  describe('styled', () => {
    it('adds the style to the element', () => {
      const s = fntags.styled({ color: 'blue' }, 'div', [])
      expect(s.style.color).eq('blue')
    })
    it('adds the style to an element with an existing style', () => {
      const s = fntags.styled({ color: 'blue' }, 'div', [{ style: { 'font-size': '10px' } }])
      expect(s.style.color).eq('blue')
      expect(s.style['font-size']).eq('10px')
    })
    it('allows overwriting the style', () => {
      const s = fntags.styled({ color: 'blue' }, 'div', [{ style: { color: 'purple' } }])
